...
BATCH_SIZE = 4096
LR         = 5e-4
L_PE       = 10     # positional encoding levels - more = captures finer detail
HIDDEN     = 256
N_LAYERS   = 4
LOG_EVERY  = 500
IMG_RESIZE = 400
...
# positional encoding - maps (u,v) to higher freq features so mlp can learn detail
...
# mlp that maps pixel coords to rgb
...
class PixelDataset(Dataset):
    def __init__(self, img_path, resize=None):
        ...
        self.coords = torch.from_numpy(coords.reshape(-1, 2))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--image", default=str(PROJECT_DIR / "test_images" / "food.png"))
    ...
    model.eval()
    with torch.no_grad():
        chunks = [model(ds.coords[i:i+8192].to(device)).cpu() for i in range(0, len(ds.coords), 8192)]
    ...
    fig.tight_layout(); fig.savefig(OUT_DIR / "comparison.png", dpi=150)
    print(f"saved -> {OUT_DIR / 'comparison.png'}")

    steps_log, psnrs = zip(*psnr_log)
    plt.figure(figsize=(7, 4))
    plt.plot(steps_log, psnrs, marker="o")
    plt.xlabel("Iteration"); plt.ylabel("PSNR (dB)"); plt.title("2D Neural Field Training"); plt.grid(True)
