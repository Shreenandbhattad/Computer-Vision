...
D     = calib["dist_coeffs"]
print(f"loaded calibration  rms={float(calib['rms']):.4f}")
print(f"K =\n{K}\n")
...
print(f"found {len(img_paths)} scan images in {SCAN_DIR}\n")
...
for p in img_paths:
    ...
    if ids is None or len(ids) == 0:
        print(f"  skip (no aruco tag visible): {p.name}")
        skipped += 1; continue

    # use the first detected tag for pose
    ...
    if not success:
        print(f"  skip (solvePnP failed): {p.name}")
        skipped += 1; continue
    ...
    # OpenCV -> OpenGL coords (flip Y and Z)
    c2w[:3, 1] *= -1
    c2w[:3, 2] *= -1
...
if len(images_list) < 5:
    raise RuntimeError(
        f"Only {len(images_list)} usable images.\n"
    )
